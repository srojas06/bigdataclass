...
# Carga los CSV 
def carga_datos():
    df_ciclistas = spark.read.csv('data/ciclista.csv', header=False, inferSchema=True)\
        .toDF('Cedula', 'Nombre', 'Provincia')
    df_rutas = spark.read.csv('data/ruta.csv', header=False, inferSchema=True)\
        .toDF('Codigo_Ruta', 'Nombre_Ruta', 'Kilometros')
    df_actividades = spark.read.csv('data/actividad.csv', header=False, inferSchema=True)\
