...
def carga_datos():
    df_ciclistas = spark.read.csv('datos/ciclistas.csv', header=False, inferSchema=True)\
        .toDF('Cedula', 'Nombre', 'Provincia')
    df_rutas = spark.read.csv('datos/rutas.csv', header=False, inferSchema=True)\
        .toDF('Codigo_Ruta', 'Nombre_Ruta', 'Kilometros')
    df_actividades = spark.read.csv('datos/actividades.csv', header=False, inferSchema=True)\
